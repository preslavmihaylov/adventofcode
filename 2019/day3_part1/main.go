...
func placeWires(grid [][]rune, input [][]string) {
	for i := 0; i < len(input); i++ {
		...
		for _, cmd := range input[i] {
			...
			row, col = placeWire(grid, dir, row, col, dist, i)
		}
	}
}

func placeWire(grid [][]rune, dir rune, row, col, dist, wireIndex int) (int, int) {
	for i := 0; i < dist; i++ {
		switch dir {
		case 'U':
			row--
		...
		if grid[row][col] != rune(0) && grid[row][col] != rune(wireIndex+'0') {
			manhattanDist := math.Abs(float64(col-colStart)) + math.Abs(float64(row-rowStart))
			minDistance = int(math.Min(float64(minDistance), manhattanDist))
