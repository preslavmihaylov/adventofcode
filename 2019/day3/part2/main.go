...
func main() {
	grid := make([][]rune, dim)
	stepsToPos = make([][]int, dim)
	for row := 0; row < len(grid); row++ {
		grid[row] = make([]rune, dim)
		stepsToPos[row] = make([]int, dim)
	}

	placeWires(grid, input)
	fmt.Println(minSteps)
	// prettyPrint(grid)
}

func placeWires(grid [][]rune, input [][]string) {
	for i := 0; i < len(input); i++ {
		row, col := rowStart, colStart
		steps := 0
		for _, cmd := range input[i] {
			dir := rune(cmd[0])
			...
			row, col, steps = placeWire(grid, dir, row, col, dist, i, steps)
		}
	}
}

func placeWire(grid [][]rune, dir rune, row, col, dist, wireIndex, steps int) (int, int, int) {
	for i := 0; i < dist; i++ {
		steps++
		switch dir {
		case 'U':
		...
		if grid[row][col] != rune(0) && grid[row][col] != rune(wireIndex+'0') && grid[row][col] != 'X' {
			minSteps = int(math.Min(float64(minSteps), float64(stepsToPos[row][col]+steps)))

			grid[row][col] = 'X'
		} else {
			grid[row][col] = rune(wireIndex + '0')
			stepsToPos[row][col] = steps
		}
	}
