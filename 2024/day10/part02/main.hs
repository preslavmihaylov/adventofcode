...
solve input = res
  where
    ...
    grouped = groupOccurrences $ filter (\x -> (snd x) /= (-1, -1)) results
    res = sum (map snd grouped)

groupOccurrences :: (Ord a) => [a] -> [(a, Int)]
groupOccurrences xs = map (\g -> (head g, length g)) (group (sort xs))
