  ...
  def to_s
    @cells.each_index do |row|
      @cells[row].each_index do |col|
        print @cells[row][col] if col > 100
      end
...
def populate_grid(grid, rocks)
  ...
  for col in 0..grid.cells[0].length - 1
    grid.cells[max_row + 2][col] = '#'
  end
end
  ...
  while true
    ...
    while true
      ...
      if grid.cells[sand_row + 1][sand_col] == '.'
        ...
        return if sand_row == 0 and sand_col == 500
